...
{
  cell_reconfigs.reserve(MAX_NOF_DU_CELLS);
}
...
bool du_param_config_procedure::handle_cell_config_updates()
{
  ...
  for (const du_cell_param_config_request& cell_to_update : request.cells) {
    ...
    mac_cell_reconfig_request cell_recfg;
    ...
    if (cell_to_update.ssb_pwr_mod.has_value() and
      ...
      cell_recfg.new_sib1_buffer        = std::move(bcch_msgs[0]);
      needs_reconfig                    = true;
    }

    if (cell_to_update.positioning.has_value()) {
    ...
    if (needs_reconfig) {
      ...
      cell_reconfigs.push_back(std::move(cell_recfg));
    }
  }
...
async_task<gnbdu_config_update_response> du_param_config_procedure::handle_f1_gnbdu_config_update()
{
  ...
  unsigned nof_sib_updates = 0;
  for (unsigned i = 0, e = changed_cells.size(); i != e; ++i) {
    if (!cell_reconfigs[i].new_sib1_buffer.empty()) {
      const du_cell_config& cell_cfg = du_cells.get_cell_cfg(changed_cells[i]);

      auto& cell_mod = f1_req.cells_to_mod.emplace_back();
      cell_mod.cell_info   = make_f1ap_du_cell_info(cell_cfg);
      cell_mod.du_sys_info = make_f1ap_du_sys_info(cell_cfg);
      ++nof_sib_updates;
    }
  }

  if (nof_sib_updates == 0) {
...
async_task<mac_cell_reconfig_response> du_param_config_procedure::handle_mac_cell_update(unsigned changed_cell_idx)
{
  ...
  return du_params.mac.cell_mng.get_cell_controller(cell_index).reconfigure(cell_reconfigs[changed_cell_idx]);
}
