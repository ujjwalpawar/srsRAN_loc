...
#define UNIX_TO_GPS_SECONDS_OFFSET (315964800ULL - 18ULL)
...
int main(int argc, char *argv[]){
    ...
    while (1) {
        if (count == number_of_gnb) {
            ...
            // Convert Unix time to GPS time (subtract offset)
            uint64_t gps_now_ns = now_ns - UNIX_TO_GPS_SECONDS_OFFSET * 1000000000ULL;
            // Protect against underflow (unlikely if system clock is recent)
            if (now_ns < UNIX_TO_GPS_SECONDS_OFFSET * 1000000000ULL) {
                fprintf(stderr, "System time is before GPS epoch; aborting\n");
                close(udp_recv_fd);
                return 1;
            }
            uint64_t start_time_ns = gps_now_ns + (uint64_t)start_delay_ms * 1000000ULL;
